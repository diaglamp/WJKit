...
@implementation WJKVStorageItem
@end

@implementation WJKVStorage {
...
- (WJKVStorageItem *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData {
    int i = 0;
    ...
    WJKVStorageItem *item = [WJKVStorageItem new];
...
- (WJKVStorageItem *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData {
    NSString *sql = excludeInlineData ? @"select key, filename, size, modification_time, last_access_time, extended_data from manifest where key = ?1;" : @"select key, filename, size, inline_data, modification_time, last_access_time, extended_data from manifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt) return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
    
    WJKVStorageItem *item = nil;
    int result = sqlite3_step(stmt);
...
- (NSMutableArray *)_dbGetItemWithKeys:(NSArray *)keys excludeInlineData:(BOOL)excludeInlineData {
    ...
    do {
        result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            WJKVStorageItem *item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"WJKVStorage init error" reason:@"Please use the designated initializer and pass the 'path' and 'type'." userInfo:nil];
    return [self initWithPath:@"" type:WJKVStorageTypeFile];
}

- (instancetype)initWithPath:(NSString *)path type:(WJKVStorageType)type {
    if (path.length == 0 || path.length > kPathLengthMax) {
        NSLog(@"WJKVStorage init error: invalid path: [%@].", path);
        return nil;
    }
    if (type > WJKVStorageTypeMixed) {
        NSLog(@"WJKVStorage init error: invalid type: %lu.", (unsigned long)type);
        return nil;
    }
    ...
    _trashQueue = dispatch_queue_create("com.xhj.cache.disk.trash", DISPATCH_QUEUE_SERIAL);
    ...
    if (![[NSFileManager defaultManager] createDirectoryAtPath:path
        ...
        ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kTrashDirectoryName]
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error]) {
            NSLog(@"WJKVStorage init error:%@", error);
            return nil;
        }
    ...
    if (![self _dbOpen] || ![self _dbInitialize]) {
        ...
        if (![self _dbOpen] || ![self _dbInitialize]) {
            [self _dbClose];
            NSLog(@"WJKVStorage init error: fail to open sqlite db.");
        }
        return nil;
    }
...
- (BOOL)saveItemWithKey:(NSString *)key value:(NSData *)value filename:(NSString *)filename extendedData:(NSData *)extendedData {
    ...
    if (_type == WJKVStorageTypeFile && filename.length == 0) {
        return NO;
    }
    ...
    if (filename.length) {
        ...
        if (_type != WJKVStorageTypeSQLite) {
...
- (BOOL)removeItemForKey:(NSString *)key {
    ...
    switch (_type) {
        case WJKVStorageTypeSQLite: {
            return [self _dbDeleteItemWithKey:key];
        } break;
        case WJKVStorageTypeFile:
        case WJKVStorageTypeMixed: {
            NSString *filename = [self _dbGetFilenameWithKey:key];
            if (filename) {
                [self _fileDeleteWithName:filename];
            }
...
- (BOOL)removeItemsToFitSize:(int)maxSize {
    ...
    do {
        ...
        for (WJKVStorageItem *item in items) {
            if (total > maxSize) {
                if (item.filename) {
...
- (WJKVStorageItem *)getItemForKey:(NSString *)key {
    if (key.length == 0) return nil;
    WJKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:NO];
    if (item) {
...
- (WJKVStorageItem *)getItemInfoForKey:(NSString *)key {
    if (key.length == 0) return nil;
    WJKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:YES];
    return item;
}
