...
/**
 Storage type, indicated where the `WJKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache),
 use WJKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
 use WJKVStorageTypeFile to get better performance.
 * You can use WJKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, WJKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    WJKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    WJKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    WJKVStorageTypeMixed = 2,
};


/**
 WJKVStorageItem is used by `WJKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface WJKVStorageItem : NSObject
...
@property (nonatomic, readonly) WJKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(WJKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is WJKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is WJKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is WJKVStorageTypeMixed, then the item.value will be saved to file
 system if the item.filename is not empty, otherwise it will be saved to sqlite.
 
 @param item  An item.
 @return Whether succeed.
 */
- (BOOL)saveItem:(WJKVStorageItem *)item;

/**
 Save an item or update the item with 'key' if it already exists.
 
 @discussion This method will save the key-value pair to sqlite. If the `type` is
 WJKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is WJKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is WJKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is WJKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable WJKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable WJKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `WJKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<WJKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `WJKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<WJKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;
