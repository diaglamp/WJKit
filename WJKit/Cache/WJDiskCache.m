...
static int64_t _WJDiskSpaceFree() {
    NSError *error = nil;
...
static void _WJDiskCacheInitGlobal() {
...
static WJDiskCache *_WJDiskCacheGetGlobal(NSString *path) {
    if (path.length == 0) return nil;
    _WJDiskCacheInitGlobal();
...
static void _WJDiskCacheSetGlobal(WJDiskCache *cache) {
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"WJDiskCache init error" reason:@"WJDiskCache must be initialized with a path. Use 'initWithPath:' or 'initWithPath:inlineThreshold:' instead." userInfo:nil];
...
- (instancetype)initWithPath:(NSString *)path
    ...
    WJDiskCache *globalCache = _WJDiskCacheGetGlobal(path);
    if (globalCache) return globalCache;
    
    WJKVStorageType type;
    if (threshold == 0) {
        type = WJKVStorageTypeFile;
    } else if (threshold == NSUIntegerMax) {
        type = WJKVStorageTypeSQLite;
    } else {
        type = WJKVStorageTypeMixed;
    }
    
    WJKVStorage *kv = [[WJKVStorage alloc] initWithPath:path type:type];
    if (!kv) return nil;
    
    _kv = kv;
    _path = path;
    _lock = dispatch_semaphore_create(1);
    _queue = dispatch_queue_create("com.xhj.cache.disk", DISPATCH_QUEUE_CONCURRENT);
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    WJKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [WJDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [WJDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    ...
    if (_kv.type != WJKVStorageTypeSQLite) {
...
- (void)_trimToFreeDiskSpace:(NSUInteger)targetFreeDiskSpace {
    ...
    int64_t diskFreeBytes = _WJDiskSpaceFree();
