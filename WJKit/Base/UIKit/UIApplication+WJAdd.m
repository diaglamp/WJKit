...
WJSYNTH_DUMMY_CLASS(UIApplication_WJAdd)

#define kNetworkIndicatorDelay (1/30.0)
@interface _WJNetworkIndicatorInfo : NSObject
@property (nonatomic, assign) NSInteger count;
@property (nonatomic, strong) NSTimer *timer;
@end

@implementation _WJNetworkIndicatorInfo
@end
...
- (BOOL)_wj_fileExistInMainBundle:(NSString *)name {
...
- (BOOL)isPirated {
    // Simulator is not from appstore
    ...
    if (![self _wj_fileExistInMainBundle:@"_CodeSignature"]) {
        return YES;
    }
    
    if (![self _wj_fileExistInMainBundle:@"SC_Info"]) {
        return YES;
    }
...
WJSYNTH_DYNAMIC_PROPERTY_OBJECT(networkActivityInfo, setNetworkActivityInfo, RETAIN_NONATOMIC, _WJNetworkIndicatorInfo *);
...
- (void)_changeNetworkActivityCount:(NSInteger)delta {
    @synchronized(self){
        dispatch_async_on_main_queue(^{
            _WJNetworkIndicatorInfo *info = [self networkActivityInfo];
            if (!info) {
                info = [_WJNetworkIndicatorInfo new];
