...
typedef NS_ENUM(NSUInteger, WJAnimatedImageType) {
    WJAnimatedImageTypeNone = 0,
    WJAnimatedImageTypeImage,
    WJAnimatedImageTypeHighlightedImage,
    WJAnimatedImageTypeImages,
    WJAnimatedImageTypeHighlightedImages,
};

@interface WJAnimatedImageView() {
    @package
    UIImage <WJAnimatedImage> *_curAnimatedImage;
...
@interface _WJAnimatedImageViewFetchOperation : NSOperation
@property (nonatomic, weak) WJAnimatedImageView *view;
@property (nonatomic, assign) NSUInteger nextIndex;
@property (nonatomic, strong) UIImage <WJAnimatedImage> *curImage;
@end

@implementation _WJAnimatedImageViewFetchOperation
- (void)main {
    __strong WJAnimatedImageView *view = _view;
    if (!view) return;
...
@implementation WJAnimatedImageView
...
- (void)setImage:(UIImage *)image {
    ...
    [self setImage:image withType:WJAnimatedImageTypeImage];
}

- (void)setHighlightedImage:(UIImage *)highlightedImage {
    if (self.highlightedImage == highlightedImage) return;
    [self setImage:highlightedImage withType:WJAnimatedImageTypeHighlightedImage];
}

- (void)setAnimationImages:(NSArray *)animationImages {
    if (self.animationImages == animationImages) return;
    [self setImage:animationImages withType:WJAnimatedImageTypeImages];
}
...
- (void)setHighlightedAnimationImages:(NSArray *)highlightedAnimationImages {
    ...
    [self setImage:highlightedAnimationImages withType:WJAnimatedImageTypeHighlightedImages];
}
...
- (id)imageForType:(WJAnimatedImageType)type {
    switch (type) {
        case WJAnimatedImageTypeNone: return nil;
        case WJAnimatedImageTypeImage: return self.image;
        case WJAnimatedImageTypeHighlightedImage: return self.highlightedImage;
        case WJAnimatedImageTypeImages: return self.animationImages;
        case WJAnimatedImageTypeHighlightedImages: return self.highlightedAnimationImages;
    }
    return nil;
}

- (WJAnimatedImageType)currentImageType {
    WJAnimatedImageType curType = WJAnimatedImageTypeNone;
    if (self.highlighted) {
        if (self.highlightedAnimationImages.count) curType = WJAnimatedImageTypeHighlightedImages;
        else if (self.highlightedImage) curType = WJAnimatedImageTypeHighlightedImage;
    }
    if (curType == WJAnimatedImageTypeNone) {
        if (self.animationImages.count) curType = WJAnimatedImageTypeImages;
        else if (self.image) curType = WJAnimatedImageTypeImage;
    }
    return curType;
}

- (void)setImage:(id)image withType:(WJAnimatedImageType)type {
    [self stopAnimating];
    if (_link) [self resetAnimated];
    _curFrame = nil;
    switch (type) {
        case WJAnimatedImageTypeNone: break;
        case WJAnimatedImageTypeImage: super.image = image; break;
        case WJAnimatedImageTypeHighlightedImage: super.highlightedImage = image; break;
        case WJAnimatedImageTypeImages: super.animationImages = image; break;
        case WJAnimatedImageTypeHighlightedImages: super.highlightedAnimationImages = image; break;
    }
    [self imageChanged];
}
...
- (void)resetAnimated {
    dispatch_once(&_onceToken, ^{
        ...
        _link = [CADisplayLink displayLinkWithTarget:[WJWeakProxy proxyWithTarget:self] selector:@selector(step:)];
        if (_runloopMode) {
...
- (void)calcMaxBufferCount {
    ...
    maxBufferCount = WJ_CLAMP(maxBufferCount, 1, 512);
...
- (void)startAnimating {
    WJAnimatedImageType type = [self currentImageType];
    if (type == WJAnimatedImageTypeImages || type == WJAnimatedImageTypeHighlightedImages) {
...
- (void)step:(CADisplayLink *)link {
    UIImage <WJAnimatedImage> *image = _curAnimatedImage;
    ...
    if (!bufferIsFull && _requestQueue.operationCount == 0) { // if some work not finished, wait for next opportunity
        _WJAnimatedImageViewFetchOperation *operation = [_WJAnimatedImageViewFetchOperation new];
        operation.view = self;
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    UIImage *image = [aDecoder decodeObjectForKey:@"WJAnimatedImage"];
    UIImage *highlightedImage = [aDecoder decodeObjectForKey:@"WJHighlightedAnimatedImage"];
    if (image) {
        self.image = image;
        [self setImage:image withType:WJAnimatedImageTypeImage];
    }
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    ...
    BOOL ani, multi;
    ani = [self.image conformsToProtocol:@protocol(WJAnimatedImage)];
    multi = (ani && ((UIImage <WJAnimatedImage> *)self.image).animatedImageFrameCount > 1);
    if (multi) [aCoder encodeObject:self.image forKey:@"WJAnimatedImage"];
    
    ani = [self.highlightedImage conformsToProtocol:@protocol(WJAnimatedImage)];
    multi = (ani && ((UIImage <WJAnimatedImage> *)self.highlightedImage).animatedImageFrameCount > 1);
    if (multi) [aCoder encodeObject:self.highlightedImage forKey:@"WJHighlightedAnimatedImage"];
}
