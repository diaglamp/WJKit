...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        WJImageDecoder *decoder = [WJImageDecoder decoderWithData:data scale:scale];
        WJImageFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    NSNumber *scale = [aDecoder decodeObjectForKey:@"WJImageScale"];
    NSData *data = [aDecoder decodeObjectForKey:@"WJImageData"];
    if (data.length) {
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    if (_decoder.data.length) {
        [aCoder encodeObject:@(self.scale) forKey:@"WJImageScale"];
        [aCoder encodeObject:_decoder.data forKey:@"WJImageData"];
    } else {
        [super encodeWithCoder:aCoder]; // Apple use UIImagePNGRepresentation() to encode UIImage.
    }
}

#pragma mark - protocol WJAnimatedImage
